...
app.put("/api/stock/:id", (req: Request, res: Response) => {
    ...
    if (resolution && "resolution" in item) (item as Tv).resolution = resolution;
    if (screenSize && "screenSize" in item) (item as Tv).screenSize = Number(screenSize);
    if (memory && "memory" in item) (item as Cellphone).memory = Number(memory);
    if (rimSize && "rimSize" in item) (item as Bike).rimSize = Number(rimSize);
    stock.remove(item);
    stock.add(item);
