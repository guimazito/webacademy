...
export async function getListaProduto(): Promise<Produto[]> {
    return apiProdutos.get<Produto[]>("/produto").then((response) => response.data);
};

export async function getListaFavoritos(): Promise<Produto[]> {
    return apiFavoritos.get<Produto[]>("/favoritos").then((response) => response.data);
};
...
export async function getProduto(id: string): Promise<Produto> {
    return apiProdutos.get<Produto>(`/produto/${id}`).then((response) => response.data);
}